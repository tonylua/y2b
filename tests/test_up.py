...
def main2():
    """
    测试使用 ffmpeg 为视频添加 ASS 字幕
    
    此函数使用 ffmpeg 命令行工具，将指定的 ASS 字幕文件添加到视频中，
    并输出到指定路径。
    """
...
async def main():
    ...
    此函数使用 YouTubeTranscriptApi 下载指定视频的字幕，
    并将其翻译为中文，然后保存为 SRT 格式文件。
    
    同时包含了 B 站视频上传的测试代码，但目前处于注释状态。
    """
